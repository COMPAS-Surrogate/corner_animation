...
def plotGTC(chains, **kwargs):
    ...
    # Set up some colors
    truthsDefaultColors = ['tab:orange',]
    truthsDefaultLS = ['--']
    colorsDict = {
                  ...
                  'lightgray': ('#d3d3d3', '#e0e0e0', '#f0f0f0'),
              }
        ...
        for i in range(nDim):  # row
            for j in range(nDim):  # column
                if j < i:
                    ...
                    ax.tick_params(direction='in', top=True, right=True, pad=1,
                                   colors=axisColor, size=1, width=.5,
                                   labelsize=6)
                    ...
                    if xTicks[j] is None:
                        # 5 ticks max
                        ax.xaxis.set_major_locator(mtik.MaxNLocator(3))
                    ...
                    if yTicks[i] is None:
                        # 5 ticks max
                        ax.yaxis.set_major_locator(mtik.MaxNLocator(3))
                        # Remove xticks that are too close to panel edge
    ...
    if do1dPlots:
        ...
        for i in range(nDim):
            ...
            # Global tick properties
            ax.tick_params(direction='in', top=True, right=True, pad=1,
                           colors=axisColor, size=1, width=.5, labelsize=6)
    ...
    if (chainLabels is not None) or (truthLabels is not None):
        ...
        if truthLabels is not None:
            ...
            for k in range(len(truthLabels)):
                ax.plot(0, 0, lw=0.5, color=truthColors[k], label=truthLabels[k],
...
def __plot1d(ax, nChains, chains1d, weights, nBins, smoothingKernel,
    ...
    plotData = []
    # With smoothing
    if smoothingKernel > 0 and SMOOTH:
    ...
    if truths1d is not None:
        for k in range(len(truths1d)):
            if truths1d[k] is not None:
                ax.axvline(truths1d[k], lw=0.5, color=truthColors[k],
                           ls=truthLineStyles[k])
    ...
    if truths2d is not None:
        for k in range(len(truths2d)):
            ...
            if truths2d[k][0] is not None:
                ax.axhline(truths2d[k][0], lw=0.5, color=truthColors[k],
                           ls=truthLineStyles[k])
            # vertical line
            if truths2d[k][1] is not None:
                ax.axvline(truths2d[k][1], lw=0.5, color=truthColors[k],
                           ls=truthLineStyles[k])
